...
from app.utils.validators import sanitizar_input_string, detectar_padroes_suspeitos
...
@operador_required
def novo_medico():
    nome = sanitizar_input_string(request.form.get("nome", ""), max_length=_MAX_NOME)
    crm = sanitizar_input_string(request.form.get("crm", ""), max_length=_MAX_CRM)
    especialidade = sanitizar_input_string(request.form.get("especialidade", ""), max_length=_MAX_ESPEC)
    contato_raw = request.form.get("contato", "").strip()[: _MAX_CONTATO + 5]
    contato = _limpar_tel(contato_raw) if contato_raw else None

    # Detecta padrões suspeitos
    if detectar_padroes_suspeitos(nome) or detectar_padroes_suspeitos(especialidade):
...
@login_required
@admin_required
@audit_critical_action('EXCLUSÃO_MÉDICO')

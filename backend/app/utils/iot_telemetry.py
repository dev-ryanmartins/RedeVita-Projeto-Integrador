...
class IoTTelemetrySimulator:
    ...
    def forcar_temperatura_critica(self, sensor_key: str, temperatura: float):
        """
        Força uma temperatura crítica em um sensor específico para simulação de alertas.
        
        Args:
            sensor_key: Chave do sensor (ex: 'geladeira_principal')
            temperatura: Temperatura crítica a ser forçada
        """
        if sensor_key not in self.SENSORES:
            raise ValueError(f"Sensor {sensor_key} não configurado")
        ...
        leitura = SensorReading(
            sensor_id=config['id'],
            temperatura=temperatura,
            umidade=random.uniform(config['umidade_min'], config['umidade_max']),
            timestamp=datetime.utcnow(),
            localizacao=config['localizacao'],
        ...
        logger.warning(
            f"Temperatura crítica forçada no sensor {config['id']}: "
            f"{temperatura}°C em {config['localizacao']}"
        )

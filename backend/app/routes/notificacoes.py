...
from app.utils.notificacoes import (
    ...
    contagem_alertas,
    medicamentos_vencidos_com_estoque,
    medicamentos_estoque_baixo,
)
...
def painel():
    ...
    mail_ok = bool(os.environ.get("MAIL_USERNAME") and os.environ.get("MAIL_PASSWORD"))
...
def enviar():
    ...
    if canal == "email":
        ...
        if resultado["ok"]:
            if resultado.get("enviado"):
                registrar_log(
                    "Alerta por E-mail",
                    f'E-mail enviado para {destinatario} — {resultado["mensagem"]}',
                )
    ...
    if resultado["ok"]:
        if resultado.get("enviado"):
            registrar_log(
                "Notificação Enviada",
                f'{canal.upper()} enviado para {telefone_limpo} — {resultado["mensagem"]}',
            )
            flash(resultado["mensagem"], "success")
        else:
            flash(resultado["mensagem"], "info")

    ...
    if request.method == 'POST':
        ...
        if usuario and verificar_senha(usuario.senha, senha):
            if usuario.ativo is False:
                ...
                registrar_log('Login Bloqueado', f'Tentativa de login de conta desativada (CPF: {cpf[:3]}***) — IP: {request.remote_addr}')
                return render_template('login.html')
            login_user(usuario)
            registrar_log('Login', f'Usuário "{usuario.nome}" ({usuario.cargo}) fez login — IP: {request.remote_addr}')
            return redirect(url_for('inventory.dashboard'))

        registrar_log('Login Falhou', f'Tentativa de login inválida (CPF: {cpf[:3] if cpf else "?"}***) — IP: {request.remote_addr}')
...
def cadastro():
    if request.method == 'POST':
        ...
        try:
            ...
            db.session.commit()
            registrar_log('Cadastro', f'Novo usuário "{nome}" ({cargo}) cadastrado — IP: {request.remote_addr}')
...
def recuperar_senha():
    if request.method == 'POST':
        ...
        flash('Se o e-mail existir no sistema, você receberá o link de recuperação em breve.', 'info')
        return redirect(url_for('auth.login'))
    ...
    if request.method == 'POST':
        ...
        if usuario:
            try:
                ...
                db.session.commit()
                registrar_log('Redefinição de Senha', f'Senha do usuário "{usuario.nome}" redefinida — IP: {request.remote_addr}')
...
@login_required
def logout():
    registrar_log('Logout', f'Usuário "{current_user.nome}" saiu do sistema — IP: {request.remote_addr}')
    logout_user()

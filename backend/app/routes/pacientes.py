...
from app.utils.validators import sanitizar_input_string, validar_cpf, detectar_padroes_suspeitos
...
@medico_required
def novo_paciente():
    nome = sanitizar_input_string(request.form.get("nome", ""), max_length=150)
    cpf_raw = request.form.get("cpf", "").strip()
    data_nasc_str = request.form.get("data_nascimento", "").strip()
    endereco = sanitizar_input_string(request.form.get("endereco", ""), max_length=300)

    # Detecta padrões suspeitos
    if detectar_padroes_suspeitos(nome) or detectar_padroes_suspeitos(endereco):
...
@login_required
@admin_required
@audit_critical_action('EXCLUSÃO_PACIENTE')
